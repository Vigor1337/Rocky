...
function CHECKSELF(SPEAKER)if SPEAKER.Name == LP.Name then return true end end
...
function OPEN_MAIN()
	...
	local FUN = {'knife', 'swastika', 'trowel', 'path giver', 'orbital strike'}
	local Y_FUN = 5
	...
	for i,v in pairs(HOLDERS.fun:GetChildren()) do
		...
		v.load.MouseButton1Down:connect(function()
			if v.Name == 'knife' then LOAD_KNIFE()
...
function LOAD_KNIFE()
    ...
    Vel = game:GetService("RunService").Heartbeat:connect(function()
            if Knife then
                Knife.Velocity = Vector3.new(Character["HumanoidRootPart"].CFrame.LookVector.X * 85, Character["HumanoidRootPart"].AssemblyLinearVelocity.Y * 7, Character["HumanoidRootPart"].CFrame.LookVector.Z * 85)
            else
                Vel:Disconnect()
            end
    end)
    ...
    attacks = {74894663, 74813494, 96559159, 96559161}
    
    local Mouse = Player:GetMouse()
    
    
    function equipped()
    if Character["Right Leg"]:FindFirstChild("Knifing") then
        Character["Right Leg"].Knifing:Destroy()
        Knife.Knifing:Destroy()
    end
    CreateWeld(Character["Right Arm"],Vector3.new(0,2,0),Vector3.new(-40,0,0),Vector3.new(0,0.55,0))
    PlayAnim(94160581,1) -- Equip Animation
    wait(.1)
    hold = PlayAnim(96559165,1) -- Holding Knife Animation
    
    anims = Mouse.Button1Down:Connect(function()
         if Playing == false then
         if Character then
             value = math.random(1, #attacks)
             picked_value = attacks[value]
             PlayAnim(picked_value,1)
             Playing = true
         else
             anims:Disconnect()
         end
    ...
    function unequipped()
        ...
        anims:Disconnect()
        Knife.Knifing:Destroy()
        Character["Right Arm"].Knifing:Destroy()
        CreateWeld(Character["Right Leg"],Vector3.new(0,0,0),Vector3.new(0,0,0),Vector3.new(0.6,0.3,0))
    end
...
function(ARGS, SPEAKER)
	...
	for i,v in pairs(PLAYERS) do
		local PCHAR = _PLAYERS[v].Character
		if PCHAR and CHECKSELF(SPEAKER) then
...
NOTIFY('Script loaded!', 50, 255, 50)